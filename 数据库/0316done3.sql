...
CREATE TABLE `contribute` (
  ...
  `actReview` bit(1) NOT NULL COMMENT '稿件审核状态',
  PRIMARY KEY (`ctbtId`),
  KEY `投稿` (`atcId`),
...
CREATE TABLE `hanfushop` (
  ...
  `shopYear` int(10) NOT NULL COMMENT '店铺认证时间',
...
CREATE TABLE `manage` (
  ...
  `manageSex` bit(1) DEFAULT NULL COMMENT '管理员性别（1男0女）',
...
insert  into `manage`(`manageId`,`manageName`,`managePassword`,`manageEmail`,`manageSex`,`managePhone`) values (1,'w','123','1234@qq.com','\0',NULL);
...
CREATE TABLE `user` (
  ...
  `userSex` bit(1) NOT NULL COMMENT '用户性别（1男0女）',
  ...
  `updateTime` date DEFAULT NULL COMMENT '用户更新时间',
  PRIMARY KEY (`userId`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8;

/*Data for the table `user` */

insert  into `user`(`userId`,`userAccout`,`userName`,`userPassword`,`userSex`,`userEmail`,`userPhone`,`userQQ`,`userImage`,`userAddress`,`userBirthday`,`userSign`,`userBackImg`,`createTime`,`updateTime`) values (1,'111111','wang','1','\0','12345678@qq.com','12345678','123456789',NULL,NULL,NULL,NULL,NULL,'2021-03-16',NULL);
