...
CREATE TABLE `hanfuactivity` (
  ...
  `acDetails` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '活动详情',
  PRIMARY KEY (`acId`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `hanfushop` (
  ...
  `shopYear` date NOT NULL COMMENT '店铺认证时间',
  `shopFans` int(11) NOT NULL COMMENT '店铺粉丝量',
  PRIMARY KEY (`shopId`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
insert  into `manage`(`manageId`,`manageName`,`managePassword`,`manageEmail`,`manageSex`,`managePhone`) values (1,'w','123','1234@qq.com',0,NULL),(2,'admin','111','',NULL,NULL);
...
CREATE TABLE `user` (
  ...
  `userAccount` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '用户账号',
  ...
  `userPhone` varchar(15) CHARACTER SET latin1 DEFAULT NULL COMMENT '用户电话',
  `userQQ` varchar(15) CHARACTER SET latin1 DEFAULT NULL COMMENT '用户QQ',
